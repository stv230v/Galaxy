...
if (!container) {
  console.error("scene-root not found");
}
...
scene.add(ambient);
const point = new THREE.PointLight(0xffffff, 2, 500);
...
const spriteMat = new THREE.SpriteMaterial({
  ...
  ), // using standard three.js example glow
  color: 0xffaa00,
  transparent: true,
  opacity: 0.7,
...
const planets = [
  ...
  },
  {
    ...
    c1: "#dcb060",
    c2: "#b89040",
    desc: "厚い雲に覆われた灼熱の惑星。地球と大きさが似ている。",
  },
  {
    name: "Earth",
    size: 0.65,
...
function createGalaxy() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const randomX =
      Math.pow(Math.random(), 3) * (Math.random() < 0.5 ? 1 : -1) * 2;
    const randomY =
      Math.pow(Math.random(), 3) * (Math.random() < 0.5 ? 1 : -1) * 2;
    const randomZ =
      Math.pow(Math.random(), 3) * (Math.random() < 0.5 ? 1 : -1) * 2;
...
function spawnUFO(ufo) {
  ...
  // Velocity
  ufo.velocity
    .subVectors(targetPos, ufo.group.position)
    .normalize()
    .multiplyScalar(12); // Speed
...
function onMouseClick(event) {
  ...
  const dist = mouseDownPos.distanceTo(
  ...
  if (intersects.length > 0) {
    ...
    if (object === sun) {
      data = {
        name: "Sun",
        desc: "太陽系の中心にある恒星。巨大なエネルギーを放出し続けている。",
      };
    }
...
function animate(now) {
  ...
  orbitGroups.forEach((o) => {
    o.group.rotation.y += o.speed * simDelta * 60;
  });

  // update moon orbit position to earth position
  if (earthIndex >= 0 && moonMesh) {
    const earthPos = planetMeshes[earthIndex].mesh.getWorldPosition(
      new THREE.Vector3()
    );
    ...
    if (isFocusing && focusedObject) {
      ...
      if (isZooming) {
        ...
        if (dist > targetDist + 1) {
          ...
          const idealPos = targetPos
            .clone()
            .add(dir.multiplyScalar(targetDist));
      ...
      if (
        camera.position.distanceTo(savedCameraPos) < 0.5 &&
        controls.target.distanceTo(savedControlsTarget) < 0.5
      ) {
        isRestoring = false;
